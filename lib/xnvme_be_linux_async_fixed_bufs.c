...
int
xnvme_be_linux_fixed_bufs_init(struct xnvme_queue *q, int opts)
{
...
int
xnvme_be_linux_fixed_bufs_poke(struct xnvme_queue *q, uint32_t max)
{
...
#ifdef NVME_URING_CMD_IO
int
xnvme_be_linux_fixed_bufs_io(struct xnvme_cmd_ctx *ctx, void *dbuf, size_t dbuf_nbytes, void *XNVME_UNUSED(mbuf),
		       size_t mbuf_nbytes)
{
...
#else
int
xnvme_be_linux_fixed_bufs_io(struct xnvme_cmd_ctx *ctx, void *dbuf, size_t dbuf_nbytes, void *mbuf,
		       size_t mbuf_nbytes)
{
	XNVME_DEBUG("FAILED: not supported, built on system without NVME_URING_CMD_IO");
...
int
xnvme_be_linux_fixed_bufs_iov(struct xnvme_cmd_ctx *ctx, struct iovec *dvec, size_t dvec_cnt,
...
struct xnvme_be_async g_xnvme_be_linux_async_fixed_bufs = {
	.id = "fixed_bufs",
#ifdef XNVME_BE_LINUX_LIBURING_ENABLED
	.cmd_io = xnvme_be_linux_fixed_bufs_io,
	.cmd_iov = xnvme_be_linux_fixed_bufs_iov,
	.poke = xnvme_be_linux_fixed_bufs_poke,
	.wait = xnvme_be_nosys_queue_wait,
	.init = xnvme_be_linux_fixed_bufs_init,
